...
class nilai_Model extends CI_Model
{
   ...
   public function countdataguru($id)
    ...
    {
        $query = "SELECT `id_penilaian`, p.id_nilai, n.id_guru, p.nilai, p.bobot_w, p.bobot_s, p.bobot_v, p.rank FROM `nilai` n join 
        `penilaian` p on n.id_nilai = p.id_nilai WHERE n.id_guru = ? AND n.id_periode = ? ORDER BY id_penilaian";
        return $this->db->query($query, array($id, $tahun))->result_array();
    }
    
    public function get_AllNilai2($id, $tahun)
    {
        $query = "SELECT `id_penilaian`, p.id_nilai, n.nilai_guru, n.id_guru, p.nilai, p.bobot_s, p.bobot_v FROM `nilai` n join 
        `penilaian` p on n.id_nilai = p.id_nilai WHERE n.id_guru = ? AND n.id_periode = ? ORDER BY id_penilaian";
        return $this->db->query($query, array($id, $tahun))->result_array();
    }
